...
class Net_MPD_Common
{
    ...
    public function connect()
    {
        ...
        $this->_connection = @fsockopen($this->connection_params['host'], $this->connection_params['port'], $errn, $errs, 10);
        if ($this->_connection) {
            ...
            if (!is_null($this->connection_params['password'])) {
            	if (!$this->runCommand('password', $this->connection_params['password'])) {
            	   $this->error = '#'.self::ACK_PASSWORD.' - Password invalid';               	
                   return false;
                }
            }
            return true;
        }
        $this->error = '#'.self::CONNECTION_FAILED.' - Error connecting: '.$errn.' ; '.$errs;               	
        return false;
    }
    ...
    function write($data)
    {
        ...
        if (!fwrite($this->_connection, $data."\n")) {
			$this->error = '#'.self::WRITE_FAILED.' - Write to MPD failed'; 
			return false;
        }
    ...
    function read()
    {
        //Are we connected?
        if (!$this->isConnected()) {
			$this->error = '#'.self::CONNECTION_NOT_OPENED.' - Not connected';
			return false;
        }
...
//                $this->runCommand('clearerror');
                ...
                $this->_current_error = array('ack' => $matches[1], 'func' => $matches[3], 'error' => $matches[4]);
			     $this->error = '#'.self::ACK_COMMAND_FAILED.' - Command Failed';
                return false;                
    ...
    public function runCommand($command, $args = array(), $parse = 0)
    {
        ...
        if ($parse !== false) {        	
            return $this->parseOutput($output, $parse,$command);
        }
        return $output;
    }
     ...
     * @return array
     */
    public function parseOutput($input, $style = 0,$command='')
    {
        if (!is_array($input)) {
            $input = array($input);
        }
    ...
    /**
     * Used to discover if a command is available
     *
     * @return  false if not
     */
    ...
    {
	$cmds = $this->runCommand('commands');
	if (isset($cmds['command']) && (array_search($cmd,$cmds['command'])!==false)) {
        return true;
    } else {
    	echo array_search($cmd,$cmds['command']);
    	return false;
    }
    } 
    ...
    {
    $status = $this->runCommand('currentsong');
    if (0 && !isset($status['status'])) {
        return false;
    }
    return $status;
    }
