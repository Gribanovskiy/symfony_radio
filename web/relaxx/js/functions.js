...
var popInfo=null;
var popAdm=new Object;var current_row=false;
...
function initPlayer(){makeTracklist();
...
function makeTracklist(){
	...
	nSize=$("tracklist").getSize();$("trackContainer").setStyle("height",nSize.size.y);
	...
	if(dNode.className.search(/empty/)==-1){
		...
		if((!dChild)||(dChild.tagName!="UL")){
			...
			if(dNode.className.search(/open/)!=-1){
				dChild.className="closed";dNode.className="selected"
			}else{
...
function refreshTracklist(b,c){
	switch(b){
		case"search":url="include/controller-database.php?"+Object.toQueryString({
			action:b,type:c.target.value,search:c.search.value
		});
		break;
		case"listPlaylistInfo":url="include/controller-playlist.php?"+Object.toQueryString({
			action:b,value:c
		});
		break;
		default:url="include/controller-database.php?action=directory&dir="+c
	}
	var a=new Json.Remote(url,{
			...
			if(e.file){
				e.file.each(function(f){
					...
					trcolumns.each(function(g){
						if(f[g]){
							d+="<td>"+((g=="Time")?convertTime(f[g]):f[g])+"</td>"}else{d+="<td></td>"
						}
					});
					d+="</tr>"
				})
	...
	if(plStatus.playlistlength==0){
		...
		if(plChanges){
			...
			plChanges.file.each(function(c){
				rclass=(a%2)?"odd":"even";newRow=makePlaylistRow(c,rclass);
...
function recalcTable(a){
	newSize=$(a).getSize();
	container=$(a).firstChild;container.setStyle("width",newSize.size.x);
...
function makePlaylistRow(a,c){
	if((!a.Artist)&&(!a.Title)){a=buildTag(a)
	}
	...
	plcolumns.each(function(d){
		if(a[d]){
			if(d=="Time"){
				row+="<td>"+convertTime(a[d])+"</td>"}else{if(d=="Pos"){
...
function showFileinfo(){
	if(current_row){
		popInfo.setStyles({
			top:$(current_row).mousey-20,left:$(current_row).mousex+20
		});
		$(current_row).removeListener("mousemove",delayPopUp);
...
function handleKeys(b,d){
	...
	var e=(!d)?new Event(b):{key:d};if((e.control)&&(e.key=="a")){
		e.stop()
	}
	if((current_row)&&(current_element=="playlist")){
		...
		switch(e.key){
			...
			case"down":if(plpos<(oldStatus.playlistlength-1)){
				controllRemote("moveSong",plpos+":"+(plpos+1),true,"playlist")
			}
			break;
			case"p":controllRemote("play",$(current_row).getAttribute("plid"),true,"playback");
			break
		}
	}switch(e.key){
		case"esc":if($("infobox").style.display=="block"){
		...
		break;
		case"1":controllRemote("prevSong","",true,"playback");
		break;
		case"2":togglePlay();
		break;
		case"3":controllRemote("nextSong","",true,"playback");
		break;
		case"r":addStream();
		break;
		case"q":vol=Math.abs(oldStatus.volume)-5;controllRemote("setVolume",vol,true,"playback");
		break;
		case"w":vol=Math.abs(oldStatus.volume)+5;controllRemote("setVolume",vol,true,"playback");
		break;
		case"c":if(current_element=="playlist"){
		...
		case"n":savePlaylist();
		break;
		case"d":if((current_element=="playlist")||(current_element=="tracklist")){
			toggleSelected(current_element,true)
		}
		break;
		case"s":case"a":if((current_element=="playlist")||(current_element=="tracklist")){
			toggleSelected(current_element,false)
		}
		break;
		case"i":if((current_row)&&(current_element=="tracklist")){
			modalBox("<h1>"+language.file+"</h1><p align='left'>"+unescape(decodeURI($(current_row).id))+"</p>",true)
		}else{
			if((plpos)&&(current_element=="playlist")){
				var c=new Json.Remote("include/controller-playlist.php?action=getPlaylistInfo&value="+plpos,{method:"get",async:"false",onComplete:function(f){
		...
		break;
		case"space":if((current_row)&&(current_element=="tracklist")){
			addSelected()
		}
		break
	}
...
function controllRemote(d,c,e,a){
	...
	var b=new Json.Remote(url,{
		method:"get",async:"false",onComplete:function(f){
			if(f=="error"){
				error=(adminName=="anonymous")?language.error_allowed:language.error_mpd;modalBox("<h3>Error</h3><p>"+error+"</p>",true)
			}else{
				if(e){
					pingMPD()
				}
...
function toggleStatus(a){
	value=(a.className.search(/marked/g)==-1)?1:0;controllRemote(a.id,value,true,"playback")
}
function togglePlay(){
...
function setCrossfade(b){
	...
	if(a>20){a=20}controllRemote("setCrossfade",a,true,"playback")
}
	...
	var a=new Json.Remote(url,{
		onComplete:function(b){
			...
			if((b)&&(b!="error")){
				i=0;
				b.each(function(c){
					$("playlistSelect").options[i]=new Option(c,c,false,true);i++
				});
	...
	$("addURL").addEvent("submit",function(b){
		...
		if(this.stream.value!=""){
			...
			var a=new Json.Remote(url,{
				onComplete:function(c){
					if(!c.error){
						controllRemote("addSong",c.url,true,"playlist");loadPlaylists()
					}
				}
			}).send()
		}
	...
	if((c.songid!=oldStatus.songid)||(!oldStatus.songid)){
		if(c.playlistlength==0){
			...
			if($("pl"+c.song)){
				...
				document.title="#"+c.song+" : "+plTrack[1].innerHTML+" : "+plTrack[2].innerHTML+" : RelaXX";document.title=document.title.replace("&amp;","&")
			}else{
				$("trackinfo").innerHTML=language.error_notracks;
...
function loginOut(openConfig){
	if(adminName=="anonymous"){
		...
		$("login").addEvent("submit",function(e){
			new Event(e).stop();
			this.send({onComplete:function(response){
				$("infobox").style.display="none";
		...
		request=new Json.Remote("include/controller-admin.php",{
			onComplete:function(){
				adminName="anonymous";
				$("menu_login").innerHTML=language.login;modalBox("<h1>"+language.logout+"</h1><p>"+language.success_logout+"</p>",true)
			}
		}).send()
	}
}
...
function showStats(){
	...
	var a=new Json.Remote(url,{
		method:"get",async:"false",onComplete:function(b){
			if(b.songs){
				modalBox("<h1>"+language.dialog_info+"</h1><div style='font-size:12px'><b>"+language.stat_current+"</b><br />"+language.stat_bit+": "+oldStatus.bitrate+" - "+language.stat_audio+": "+oldStatus.audio+"<br /><b>"+language.stat_db+"</b><br />"+language.stat_songs+": "+b.songs+" - "+language.stat_artists+": "+b.artists+" - "+language.stat_albums+": "+b.albums+"<br />"+language.stat_playtime+" (hh:mm:ss) : "+convertTime(b.db_playtime)+"</div>",true)
			}
		}
	}).send()
}
...
function modalBox(d,c,b,a){
	...
	if(Wwidth==0){
		Wwidth=document.body.offsetWidth;Wheight=document.body.offsetHeight
	}
	$("infobox_content").innerHTML=d;
	$("infobox").style.top=Math.round((Wheight/2)-(a/2))+"px";
	$("infobox").style.left=Math.round((Wwidth/2)-(b/2))+"px";$("infobox").style.height=a+"px";
	$("infobox").style.width=b+"px";$("infobox_close").style.display=(c==false)?"none":"block";
